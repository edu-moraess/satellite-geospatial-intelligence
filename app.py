...
from ui.status import render_pipeline_status
...
# AOI SELECTION MAP
# ============================================================
# Lets the user draw a polygon/rectangle instead of relying
# only on latitude/longitude/area size. Lat/lon/area size
# above remain a valid fallback - if nothing is drawn, the
# search below falls back to them exactly as before.

st.divider()

render_section(
    "Area of Interest — Map Selection",
    description=(
        "Optional: draw a polygon or rectangle to define the "
        "exact search area. If nothing is drawn, the "
        "latitude/longitude/area size in the sidebar are used."
    ),
    icon="🗺️",
)
...
if drawn_aoi:
    ...
    if st.button(
        "🗑️ Clear drawn AOI (use lat/lon instead)"
    ):
    ...
    st.caption(
        "ℹ️ No AOI drawn yet — search will use the "
        "latitude/longitude/area size from the sidebar."
    )
...
if data:
    ...
    render_section(
        ...
        description=(
            "Sentinel-2 L2A • active dataset for analysis"
        ),
        icon="🛰️",
    )
    ...
    # RASTER VALIDATION
    # ========================================================
    # Every band is checked here, right after alignment and
    # before any pixel-wise math (RGB, spectral indices,
    # change detection). This is the single validation gate
    # requested for the platform: shape, finiteness and grid
    # consistency are guaranteed before any operation below
    # can raise band_a + band_b style errors.
    ...
    try:
        ...
        st.error(
            "❌ Raster invalid — one of the spectral bands "
            "failed validation and cannot be used safely."
        )
    ...
    render_section(
        ...
        description=(
            "Rule-based multispectral baseline using "
            "NDVI, NDWI and NDBI"
        ),
        icon="🗺️",
    )
...
render_section(
    "Change Detection",
    description=(
        "Compare two Sentinel-2 observations of the same "
        "area to identify spectral changes over time"
    ),
    icon="🛰️",
)
        ...
        if before_item.id == after_item.id:
            ...
            if (
                ...
                st.warning(
                    ...
                    "the reverse time direction — swap "
                    "the selection if that is not intended."
                )
            ...
            try:
                ...
                st.error(
                    ...
                    "cannot be compared. The two scenes are "
                    "not on a compatible spatial grid."
                )
...
if change_result:
    ...
    info_card(
        ...
        change_result["index_name"],
        description=(
            f"Data A: {change_result['before_id'][:28]}  →  "
            f"Data B: {change_result['after_id'][:28]}"
        ),
    )
...
Geospatial coordinates
        ...
        if detections:
            ...
            try:
                ...
                # ================================================
                # GEOREFERENCED EXPORT (GeoJSON)
                # ================================================
                # detection_rgb shares the pixel grid of b04
                # (the reference band), so m04's transform/CRS
                # correctly maps detection pixel coordinates to
                ...
                try:
                    ...
                    detections_gdf = georeference_detections(
                        detections,
                    ...
                    st.download_button(
                        "⬇️ Export Data (GeoJSON)",
                        data=geojson_bytes,
                        file_name=(
                            f"{data['scene_id']}_detections.geojson"
                        ),
                    ...
                    st.caption(
                        f"{len(detections_gdf)} georeferenced "
                        "detections, WGS84 (EPSG:4326)."
                    )
...
render_section(
    "Project Pipeline",
    description="Live status of each processing stage",
    icon="🚀",
)

render_pipeline_status(
    scene_available=data is not None,
